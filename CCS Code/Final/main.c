...
};//values from specifications or ti's calculator
...
int main(void)
{
    ...
    UART_transmitData(EUSCI_A2_BASE, '|'); //refresh to fix issues
    UART_transmitData(EUSCI_A2_BASE, '-');
    //config rtc
    init_toggle();//set data values needed
    config_rtc();//start rtc
}
