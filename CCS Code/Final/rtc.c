...
void init_toggle(void){
    hourt=48;//sets values needed for counter
    houro=52;
    mint=53;
    mino=53;
    togset=1;
...
void counter(void){
    if(start==0){//regular increment
            if(mino<57){
                mino++;
            }
            else if(mino==57 && mint<53){//minute tens rollover
                mino=48;
                mint++;
            }
            else if(mino==57 && mint==53 && houro<57 && hourt==48){//hour ones rollover
                mino=48;
                mint=48;
                houro++;
            }
            else if(mino==57 && mint==53 && houro==57 && hourt==48){//10 rollover
                mino=48;
                mint=48;
                houro=48;
                hourt=49;
            }
            else if(mino==57 && mint==53 && houro<50 && hourt==49){//increment
                mino=48;
                mint=48;
                houro++;
            }
            else if(mino==57 && mint==53 && houro==50 && hourt==49){//rollover and add to/start day toggle
                mino=48;
                mint=48;
                houro=49;
                hourt=48;
            ...
            if(day_tog==2 && dayo<57){//day increment
                dayo++;
                day_tog=0;
            }
            else if(day_tog==2 && dayo==57 && dayt<50){//day tens rollover
                dayo=49;
                dayt++;
                day_tog=0;
            }
            else if(day_tog==2 && dayo==57 && dayt==50){//month rollover
                dayo=49;
                dayt=48;
                month_tog=1;
                day_tog=0;
            }
            if(month_tog==1 && montht==48 && montho<57){//month increment
                montho++;
                month_tog=0;
            }
            else if(month_tog==1 && montht==48 && montho==57){//month rollover
                montho=48;
                montht=49;
                month_tog=0;
            }
            ...
            else if(month_tog==1 && montht==49 && montho==50){//year rollover
                year_tog=1;
                month_tog=0;
            }
            if(year_tog==1 && yearo<57){//year increment
                yearo++;
                year_tog=0;
            }
            ...
            if(year_tog==1 && yearo==57 && yeart==57){//year reset rollover
                yearo=48;
                yearo=48;
                year_tog=0;
            }
    }
    if(start>0){//accounts for interrupts sent during config
        UART_transmitData(EUSCI_A2_BASE, '|');
        UART_transmitData(EUSCI_A2_BASE, '-');
            start--;
        }
display();//dipslays data
}

void display(void){//transmits the data
            UART_transmitData(EUSCI_A2_BASE, '|');
            UART_transmitData(EUSCI_A2_BASE, '-');
